...
if ( ! function_exists( 'reconasia_generate_css' ) ) {
	 ...
	 */
	function reconasia_generate_css( $selector, $style, $value, $prefix = '', $suffix = '', $echo = true ) {
...
if ( ! function_exists( 'reconasia_get_customizer_css' ) ) {
	 ...
	 */
	function reconasia_get_customizer_css( $type = 'front-end' ) {

		// Get variables.
		$body              = sanitize_hex_color( reconasia_get_color_for_area( 'content', 'text' ) );
		$body_default      = '#000000';
		$secondary         = sanitize_hex_color( reconasia_get_color_for_area( 'content', 'secondary' ) );
		$secondary_default = '#6d6d6d';
		$borders           = sanitize_hex_color( reconasia_get_color_for_area( 'content', 'borders' ) );
		$borders_default   = '#dcd7ca';
		$accent            = sanitize_hex_color( reconasia_get_color_for_area( 'content', 'accent' ) );
		$accent_default    = '#cd2653';

		// Header.
		$header_footer_background         = sanitize_hex_color( reconasia_get_color_for_area( 'header-footer', 'background' ) );
		...
		*/
		...
		if ( 'front-end' === $type ) {
			...
			$elements_definitions = reconasia_get_elements_array();
			foreach ( $elements_definitions as $context => $props ) {
				foreach ( $props as $key => $definitions ) {
					foreach ( $definitions as $property => $elements ) {
						...
						$val = reconasia_get_color_for_area( $context, $key );
						if ( $val ) {
							reconasia_generate_css( implode( ',', $elements ), $property, $val );
						}
					}
				}
			}
			...
			if ( $cover && $cover !== $cover_default ) {
				reconasia_generate_css( '.overlay-header .header-inner', 'color', $cover );
				reconasia_generate_css( '.cover-header .entry-header *', 'color', $cover );
			}
			...
			if ( $accent && $accent !== $accent_default ) {
				reconasia_generate_css( '.has-accent-color, .editor-styles-wrapper .editor-block-list__layout a, .editor-styles-wrapper .has-drop-cap:not(:focus)::first-letter, .editor-styles-wrapper .wp-block-button.is-style-outline .wp-block-button__link, .editor-styles-wrapper .wp-block-pullquote::before, .editor-styles-wrapper .wp-block-file .wp-block-file__textlink', 'color', $accent );
				reconasia_generate_css( '.editor-styles-wrapper .wp-block-quote', 'border-color', $accent, '' );
				reconasia_generate_css( '.has-accent-background-color, .editor-styles-wrapper .wp-block-button__link, .editor-styles-wrapper .wp-block-file__button', 'background-color', $accent );
			}

			// Background color.
			if ( $background && $background !== $background_default ) {
				reconasia_generate_css( '.editor-styles-wrapper', 'background-color', '#' . $background );
				reconasia_generate_css( '.has-background.has-primary-background-color:not(.has-text-color),.has-background.has-primary-background-color *:not(.has-text-color),.has-background.has-accent-background-color:not(.has-text-color),.has-background.has-accent-background-color *:not(.has-text-color)', 'color', '#' . $background );
			}
			...
			if ( $borders && $borders !== $borders_default ) {
				...
				reconasia_generate_css( '.editor-styles-wrapper .wp-block-table caption, .editor-styles-wrapper .wp-block-table.is-style-stripes tbody tr:nth-child(odd)', 'background-color', $borders );
			}

			// Text color.
			if ( $body && $body !== $body_default ) {
				reconasia_generate_css( 'body .editor-styles-wrapper, .editor-post-title__block .editor-post-title__input, .editor-post-title__block .editor-post-title__input:focus', 'color', $body );
			}
			...
			if ( $header_footer_background && $header_footer_background !== $header_footer_background_default ) {
				reconasia_generate_css( '.editor-styles-wrapper .wp-block-pullquote::before', 'background-color', $header_footer_background );
			}
			...
			// Accent color.
			if ( $accent && $accent !== $accent_default ) {
				reconasia_generate_css( 'body#tinymce.wp-editor.content a, body#tinymce.wp-editor.content a:focus, body#tinymce.wp-editor.content a:hover', 'color', $accent );
				reconasia_generate_css( 'body#tinymce.wp-editor.content blockquote, body#tinymce.wp-editor.content .wp-block-quote', 'border-color', $accent, '', ' !important' );
			...
			// Background color.
			if ( $background && $background !== $background_default ) {
				reconasia_generate_css( 'body#tinymce.wp-editor.content', 'background-color', '#' . $background );
			}

			// Text color.
			if ( $body && $body !== $body_default ) {
				reconasia_generate_css( 'body#tinymce.wp-editor.content', 'color', $body );
			}
			...
			// Borders color.
			if ( $borders && $borders !== $borders_default ) {
				reconasia_generate_css( 'body#tinymce.wp-editor.content pre, body#tinymce.wp-editor.content hr, body#tinymce.wp-editor.content fieldset,body#tinymce.wp-editor.content input, body#tinymce.wp-editor.content textarea', 'border-color', $borders );
			}
		}
